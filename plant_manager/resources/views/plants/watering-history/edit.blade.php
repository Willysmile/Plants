...
<head>
  ...
  <title>Éditer l'arrosage - {{ $plant->name }}</title>
...
<div class="container mx-auto px-4 py-6 max-w-2xl">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('plants.watering-history.update', [$plant, $wateringHistory]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500 @error('amount') border-red-500 @enderror" 
                    ...
                    id="amount" 
                    name="amount" 
                    value="{{ old('amount', $wateringHistory->amount) }}" 
            ...
            <div class="mb-4">
                ...
                <textarea class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500 @error('notes') border-red-500 @enderror" 
                    ...
                    name="notes" 
                    rows="4" 
                    placeholder="Observations, remarques...">{{ old('notes', $wateringHistory->notes) }}</textarea>
                @error('notes')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
