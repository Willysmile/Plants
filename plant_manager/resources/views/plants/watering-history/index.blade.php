...
<head>
  ...
  <title>{{ $plant->name }} - Historique d'arrosage</title>
...
<div class="container mx-auto px-4 py-6">
            ...
            @foreach($histories as $history)
                <div class="bg-white rounded-lg shadow-md p-4">
                    <div class="flex justify-between items-start">
                        <div>
                            <p class="text-sm text-gray-500">{{ $history->watering_date->format('d/m/Y à H:i') }}</p>
                            @if($history->amount)
                        ...
                        <div class="space-x-2">
                            <a href="{{ route('plants.watering-history.edit', [$plant, $history]) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-3 rounded text-sm">
                                Éditer
                            </a>
                            <form action="{{ route('plants.watering-history.destroy', [$plant, $history]) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
    ...
    @else
        <div class="bg-gray-100 border border-gray-300 text-gray-700 px-4 py-3 rounded">
            Aucun historique d'arrosage enregistré.
        </div>
    @endif
</div>
