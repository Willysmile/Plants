...
<head>
  ...
  <title>{{ $plant->name }} - Historique de rempotage</title>
...
<div class="container mx-auto px-4 py-6">
            ...
            @foreach($histories as $history)
                <div class="bg-white rounded-lg shadow-md p-4">
                    <div class="flex justify-between items-start">
                        <div>
                            <p class="text-sm text-gray-500">{{ $history->repotting_date->format('d/m/Y à H:i') }}</p>
                            @if($history->old_pot_size)
                                <p class="text-gray-700"><span class="font-semibold">Ancien pot :</span> {{ $history->old_pot_size }}</p>
                            @endif
                            <p class="text-gray-700"><span class="font-semibold">Nouveau pot :</span> {{ $history->new_pot_size }}</p>
                            @if($history->soil_type)
                                <p class="text-gray-700"><span class="font-semibold">Type de terreau :</span> {{ $history->soil_type }}</p>
                            @endif
                            @if($history->notes)
                        ...
                        <div class="space-x-2">
                            <a href="{{ route('plants.repotting-history.edit', [$plant, $history]) }}" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-3 rounded text-sm">
                                Éditer
                            </a>
                            <form action="{{ route('plants.repotting-history.destroy', [$plant, $history]) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
    ...
    @else
        <div class="bg-gray-100 border border-gray-300 text-gray-700 px-4 py-3 rounded">
            Aucun historique de rempotage enregistré.
        </div>
    @endif
</div>
