...
@section('title', 'Ajouter un type d\'engrais')
...
<div class="bg-white rounded-lg shadow-md p-6 max-w-2xl">
  <div class="flex items-center mb-6">
    ...
    <h1 class="text-3xl font-bold text-gray-900 ml-4">Ajouter un type d'engrais</h1>
  </div>

  <form action="{{ route('fertilizer-types.store') }}" method="POST" class="space-y-4">
    @csrf
    ...
    <div>
      ...
      <select id="unit" 
        ...
        <option value="ml" @selected(old('unit') === 'ml')>ml (millilitres)</option>
        <option value="g" @selected(old('unit') === 'g')>g (grammes)</option>
        <option value="nombre" @selected(old('unit') === 'nombre')>Nombre (cuillères, gouttes, etc.)</option>
      </select>
      @error('unit')
        <span class="text-red-500 text-sm">{{ $message }}</span>
      @enderror
    ...
    <div>
      ...
      <textarea id="description" 
        ...
        rows="4"
        class="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500 @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
      @error('description')
        <span class="text-red-500 text-sm">{{ $message }}</span>
      @enderror
    </div>
